...
#region Imports
. "$PSScriptRoot\Modules\LoadModule.ps1" -ModuleNames @("Common", "Common.UserFolders", "Network") -Force -Verbose | Out-Null
#endregion
...
function InstallApps {
    ...
    $objects = GetConfigObjects -ConfigName "Users.$UserName.Applications"
...
function InstallMsvcrt {
    ...
    foreach ($item in $items) {
        winget install --exact --silent --id $item --accept-source-agreements
    }
}
...
function AddRegFiles {
    ...
    $objects = GetConfigObjects -ConfigName "Users.$UserName.RegFiles"
